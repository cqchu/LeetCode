...
public:
    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2)
    {
        ...
        for(int i=0; i<(nums1.size()+nums2.size()-1)/2; i++)
        {
            if(idx1 < nums1.size() && (idx2 >= nums2.size() || nums1[idx1] <= nums2[idx2]))
                ++idx1;
            else if(idx2 < nums2.size() && (idx1 >= nums1.size() || nums1[idx1] > nums2[idx2]))
                ++idx2;
        }
    ...
    *****************************************************************/
    ...
    // 要想复杂度降低为O(log(m+n)), 逐元素合并数组然后查找最中间元素显然不行, 此时需要更有效的做法
    // 现在考虑是否可以在此已排序的数组中二分查找目标元素, 但是同时在两个数组中二分查找显然也是不可行的
    // leetcode给的solution时先确定nums1和nums2中目标元素下标间恒等关系, 然后在一个nums中二分查找
    // 此恒等关系即(idx1 + idx2) * 2 = nums1.size + nums2.size(不考虑特殊的边界情况)
    // 且 min(big part) >= max(small part), big part即idx1右边和idx2右边组成的部分, small part则相反
    // 即nums2[idx2+1] > nums1[idx1] && nums1[idx1+1] > nums2[idx2]
    // 而作者考虑问题时把大于median的数看做一个整体, 小于median的看做一个整体
    // 而非把两个数组分别看为一个整体
