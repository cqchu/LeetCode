    ...
    int longestConsecutive(vector<int>& nums) {
        ...
        for (auto x: nums) {
            if (hash[x] == 0) { // not visited this num
                ...
                if (hash[x-1] != 0) // connect with left sequence, find the left endpoint of the new sequence
                    leftPoint = x-hash[x-1];
                if (hash[x+1] != 0) // connect with right sequence, find the right endpoint of the new sequence
