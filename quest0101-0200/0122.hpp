...
the stock multiple times). Note: You may not engage in multiple transactions at
...
class Solution {
public:
    // buy in the local minimum, sell in the local maximum
    int maxProfit(vector<int> &prices) {
        int totalProfit = 0, buyPrice = 0;
        ...
        if (prices[prices.size() - 1] > prices[prices.size() - 2])
            totalProfit = totalProfit + prices[prices.size() - 1] - buyPrice;
