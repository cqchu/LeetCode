...
/* Construct a graph, the vertexes are the words, two vertexes are connected if they have only a different letter. Then the shortest path between the beginWord and the endWord is the answer
    ...
    Because the weight of all edges are the same, so we can solve it by birection bfs
*/

class Solution {
public:
    void construct(unordered_multimap<string, string> &path, vector<vector<string>> &res, string &curWord) {
    ...
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        ...
        while (!beginSet.empty() && !endSet.empty()) {
            ...
            unordered_set<string> &curSet = beginSet.size() < endSet.size() ? beginSet : endSet;
            unordered_set<string> &oppoSet = beginSet.size() < endSet.size() ? endSet : beginSet;
            unordered_multimap<string, string> &curPath = beginSet.size() < endSet.size() ? beginPath : endPath;
